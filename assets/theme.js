    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateProductPrices(newVariant, previousVariant) {
        ...
        if (!newVariant) {
          ...
          var wirePriceContainer =  document.querySelector("#wirePercent");
          var price = newVariant['price'];
          var comparePrice = newVariant['compare_at_price'];
          var creditPriceContainer = document.querySelector("#creditPercent");
          var new_code_text = document.querySelector("#priceTextJson").innerText;
          if(wirePriceContainer && typeof wirePriceContainer != "undefined"){
            var percent = price * (wirePriceContainer.getAttribute("data-price")/100.0);
            //console.log(price+percent);
            wirePriceContainer.querySelector(".price").innerHTML = Currency.formatMoney(price+percent, window.theme.moneyFormat);
            wirePriceContainer = wirePriceContainer.cloneNode(true)
          }
          productPrices.innerHTML = '';

          if(creditPriceContainer && typeof creditPriceContainer != "undefined"){
            var percent = price * (creditPriceContainer.getAttribute("data-price")/100.0);
            //console.log(price+percent);
            creditPriceContainer.querySelector(".price").innerHTML = Currency.formatMoney(price+percent, window.theme.moneyFormat);
          }
          ...
          if (newVariant['compare_at_price'] > newVariant['price']) {
            ...
            productPrices.innerHTML += '<p id = "priceText" class = "price price--highlight">'+new_code_text+'</p>';
          } else {
           ...
           productPrices.innerHTML += '<p id = "priceText" class = "price">'+new_code_text+'</p>'
          }
          if(wirePriceContainer){
            productPrices.appendChild(wirePriceContainer)
          }
      ...
      value: function _updateAddToCartButton(newVariant) {
        var addToCartButtonElement = this.element.querySelector('.product-form__add-button'),
            ...
            poButtonElement = this.element.querySelector('.spurit-po-wrapper');
        var poButton, poMessage, notifyMeButton;
        if(poButtonElement){
           poButton = poButtonElement.querySelector('.pre-order-custom');
           poMessage = poButtonElement.querySelector('.pre-order-2-message');
           notifyMeButton = poButtonElement.querySelector('.notify-me-2-button');
        }
        ...
        if (!newVariant) {
          ...
          if(poButtonElement){
            poMessage.style.display = 'none';
            poButton.style.height = '48px';
            poButton.style.background = '#8a9297';
            poButton.style.color = '#ffffff';
            poButton.style.borderRadius = '2px';
            poButton.innerHTML = window.languages.productFormUnavailable;
            poButton.setAttribute('disabled', 'disabled');
